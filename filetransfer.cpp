...
int FileTransfer::startRecvFileServer(QString userName, QString filePath)
{
    ...
    mProgress->setWindowTitle(QString("从%0食物下载到%1").arg(userName).arg(filePath));
...
int FileTransfer::startSendFileServer(QString userName, QString filePath)
{
    ...
    // 开启进度条窗口
    mProgress = new QProgressDialog();
    mProgress->setWindowTitle(QString("上传文件%0到%1食物").arg(filePath).arg(userName));
    mProgress->setMinimumSize(500,mProgress->height());
    mProgress->open(this, SLOT(close()));
...
void FileTransfer::recvData()
{
    ...
    if (mHeader.len == 0) {
        if ((unsigned int)buf->size() >= sizeof(FileHeader)) {
            ...
            // 设置进度条
            mProgress->setRange(0, (int)mHeader.len);
        }
    }

    if (mHeader.len > 0 && buf->size() > 0) {
        // 增加已写入数据
        _curWritten += (unsigned int)buf->size();
        ...
        // 更新进度条
        mProgress->setValue((int)_curWritten);
    }
...
{
    // 发送文件数据
    unsigned int len = (unsigned int)mFile.size();
    unsigned int pos = 0;
    unsigned int packetSize = 800;

    // 设置进度条范围
    mProgress->setRange(0, (int)len);

    while (pos < len) {
        unsigned int sendSize = pos+packetSize < len ? packetSize : len-pos;
